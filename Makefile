...
gtk-test: gtk-test.c paths.c paths.h mptoraster.c mptoraster.h common.c common.h
	gcc -Wall `pkg-config --cflags gtk+-3.0` gtk-test.c paths.c common.c `pkg-config --libs gtk+-3.0` -L mplib -l mplib -l kpathsea -l mputil -lcairo -lpixman -lpng -lz -lmpfr -lgmp -lm -o gtk-test
